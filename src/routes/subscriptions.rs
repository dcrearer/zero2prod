...
impl std::fmt::Display for StoreTokenError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        while storing a subscription token"
        )
    }
}
...
pub async fn subscribe(
    ...
    send_confirmation_email(&email_client, &new_subscriber, &base_url.0, &subscriber_token)
        .await
...
#[tracing::instrument(
    name = "Send a confirmation to a new subscriber",
...
async fn send_confirmation_email(
    ...
    let send_email_request = SendEmailRequest {
        recipient: new_subscriber.email.clone(),
